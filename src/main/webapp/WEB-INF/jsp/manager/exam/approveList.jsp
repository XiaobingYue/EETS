    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="choose">请选择审批结果</h4>
            </div>
            <div class="modal-body">
                <form id="taskForm"
                      action="${APP_PATH}/examController/approve.do" method="post"
                      role="form">
...
<script type="text/javascript">
    ...
    $("#releaseBtn").click(function () {
        ...
        if (options.val() == 0 || options.length == 0) {
            layer.msg("请选择审批结果", {time: 1000, icon: 5, shift: 6});
            return;
        }
        $("#examId").val(_id);
        var loadingIndex = -1;
        ...
        $("#taskForm").ajaxSubmit({
            ...
            success: function (result) {
                ...
                if (result.success) {
                    layer.msg("审批成功", {time: 1000, icon: 6}, function () {
                        window.location.href = "${APP_PATH}/examController/toApprove.do?pageNo=${param.pageNo}";
                    });
                } else {
                    layer.msg("审批成功" + result.data, {time: 1000, icon: 5, shift: 6});
                }
            }
        });
    });
    ...
    function pageQuery(pageNo) {
        ...
        $.ajax({
            url: "${APP_PATH}/examController/queryUnApproveList.do",
            type: "POST",
            data: obj,
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    $.each(examList, function (i, exam) {
                        ...
                        if (exam.state == 4) {
                            content = content + '      <button type="button" title="审核" onclick="setId(' + exam.id + ')" class="btn btn-success btn-xs tooltip-test" data-toggle="modal" data-target="#myModal1"><i class=" glyphicon glyphicon-ok"></i></button>';
                        }
