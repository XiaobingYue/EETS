        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-inline" textBook="form" style="float:left;">
                        ...
                        <button type="button" class="btn btn-warning" onclick="queryTextBook()"><i
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="taskForm"
                      action="${APP_PATH}/textBookController/releaseTask.do" method="post"
                      role="form">
...
<script type="text/javascript">
    ...
    function pageQuery(pageNo) {
        ...
        $.ajax({
            url: "${APP_PATH}/textBookController/queryTextBookList.do",
            type: "POST",
            data: obj,
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    var content = "";
                    $.each(textBookList, function (i, textBook) {
                        content = content + '<tr>';
                        content = content + '  <td>' + (i + 1) + '</td>';
                        content = content + '  <td><input type="checkbox" value="' + textBook.id + '"></td>';
                        content = content + '  <td>' + textBook.name + '</td>';
                        content = content + '  <td>' + textBook.publisher + '</td>';
                        content = content + '  <td>' + textBook.courseName + '</td>';
                        content = content + '  <td>' + textBook.taskName + '</td>';
                    ...
                    layer.msg("教材分页查询数据失败", {time: 1000, icon: 5, shift: 6});
                }
            }
        });
    }
    ...
    function deleteTextBook(id, name) {
        layer.confirm("删除教材: " + name + ", 是否继续？", {icon: 3, title: '提示'}, function (cindex) {
            // 删除数据
            $.ajax({
                url: "${APP_PATH}/textBookController/delete.do",
                type: "POST",
                data: {
                    id: id
                },
                success: function (result) {
                    if (result.success) {
                        layer.msg("教材信息删除成功", {time: 1000, icon: 6}, function () {
                            pageQuery(1);
                        });
                    } else {
                        layer.msg(result.data, {time: 1000, icon: 5, shift: 6});
    ...
    function allSel(obj) {
        ...
        var textBookBox = $("tbody :checkbox");

        $.each(textBookBox, function (i, n) {
            n.checked = flg;
        });
    }

    function deleteTextBooks() {
        ...
        if (checkBox.length == 0) {
            ...
            layer.confirm("删除选择的角色数据, 是否继续？", {icon: 3, title: '提示'}, function (cindex) {
                ...
                $.ajax({
                    url: "${APP_PATH}/manager/textBook/deletes.do",
                    type: "POST",
                    data: obj,
                    success: function (result) {
                        if (result.success) {
