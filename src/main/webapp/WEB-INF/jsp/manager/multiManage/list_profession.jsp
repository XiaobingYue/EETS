    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="professionForm" role="form">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="professionForm1" role="form">
                    <div class="form-group">
                        <label>专业名称</label>
...
<script type="text/javascript">
    ...
    function pageQuery(pageNo) {
        ...
        $.ajax({
            url: "${APP_PATH}/professionController/queryProfessionList.do",
            type: "POST",
            data: obj,
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    var professionList = pageObj.data;
                    ...
                    var content = "";
                    $.each(professionList, function (i, profession) {
                        content = content + '<tr>';
                        content = content + '  <td>' + (i + 1) + '</td>';
                        content = content + '  <td><input type="checkbox" value="' + profession.id + '"></td>';
                        content = content + '  <td>' + profession.name + '</td>';
                        content = content + '  <td>';
                        content = content + '      <button type="button" data-toggle="modal" data-target="#editModal" onclick="setName('+profession.id+',\''+profession.name+'\')"  class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                        content = content + '	   <button type="button" onclick="deleteProfession(' + profession.id + ', \'' + profession.name + '\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                        content = content + '  </td>';
                        content = content + '</tr>';
                    });
                    $("tbody").html(content);
                    ...
                    layer.msg("专业分页查询数据失败", {time: 1000, icon: 5, shift: 6});
                }
            }
        });
    }
    ...
    function deleteProfession(id, name) {
        layer.confirm("删除专业: " + name + ", 是否继续？", {icon: 3, title: '提示'}, function (cindex) {
            // 删除数据
            $.ajax({
                url: "${APP_PATH}/professionController/deleteProfession.do",
                type: "POST",
                data: {
                    id: id
                },
                success: function (result) {
                    if (result.success) {
                        layer.msg("专业信息删除成功", {time: 1000, icon: 6}, function () {
                            pageQuery(1);
                        });
                    } else {
    ...
    function allSel(obj) {
        ...
        var professionBox = $("tbody :checkbox");

        $.each(professionBox, function (i, n) {
            n.checked = flg;
        });
    }
    ...
    $("#updateBtn").click(function () {
        ...
        // 提交表单
        $.ajax({
            url: "${APP_PATH}/professionController/editProfession.do",
            type: "POST",
            data: {
                "name": $("#name1").val(),
                "id": _professionId
            ...
            success: function (result) {
                layer.close(loadingIndex);
                if (result.success) {
                    layer.msg("专业信息修改成功", {time: 1000, icon: 6}, function () {
                        window.location.href = "${APP_PATH}/professionController/toProfessionList.do?pageNo=${param.pageNo}";
                    });
                } else {
                    layer.msg("专业信息修改失败", {time: 1000, icon: 5, shift: 6});
                }
            }
        });
    });
    ...
    $("#saveBtn").click(function () {
        ...
        // 提交表单
        $.ajax({
            url: "${APP_PATH}/professionController/addProfession.do",
            type: "POST",
            data: {
                "name": $("#name").val()
            },
            ...
            success: function (result) {
                layer.close(loadingIndex);
                if (result.success) {
                    layer.msg("专业信息增加成功", {time: 1000, icon: 6}, function () {
                        window.location.href = "${APP_PATH}/professionController/toProfessionList.do";
                    });
                } else {
                    layer.msg("专业信息增加失败", {time: 1000, icon: 5, shift: 6});
                }
            }
        });
    });
