        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-inline" syllabus="form" style="float:left;">
                        <div class="form-group has-feedback">
                            <div class="input-group">
                                <div class="input-group-addon">查询条件</div>
                                <input id="queryText" class="form-control has-success" type="text" style="width: 310px"
                                       placeholder="可按大纲、课程名称、课程代码模糊查询">
                            </div>
                        </div>
                        <button type="button" class="btn btn-warning" onclick="querysyllabus()"><i
                                class="glyphicon glyphicon-search"></i> 查询
                        </button>
                    </form>
                    <button type="button" class="btn btn-danger" style="float:right;margin-left:10px;"
                            onclick="deletesyllabuss()"><i class=" glyphicon glyphicon-remove"></i> 删除
                    </button>
                    <button type="button" class="btn btn-primary tooltip-test" data-toggle="tooltip" titleS="新增大纲"
                            style="float:right;"
                            onclick="window.location.href='${APP_PATH}/syllabusController/toAddSyllabusPage.do'"><i
...
<script type="text/javascript">
    ...
    initUpload("file", '${APP_PATH}/syllabusController/uploadPdf.do');
    ...
    function pageQuery(pageNo) {
        ...
        $.ajax({
            url: "${APP_PATH}/syllabusController/querySyllabusList.do",
            type: "POST",
            data: obj,
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    var content = "";
                    $.each(syllabusList, function (i, syllabus) {
                        content = content + '<tr>';
                        content = content + '  <td>' + (i + 1) + '</td>';
                        content = content + '  <td><input type="checkbox" value="' + syllabus.id + '"></td>';
                        content = content + '  <td>' + syllabus.name + '</td>';
                        content = content + '  <td>' + syllabus.version + '</td>';
                        content = content + '  <td>' + syllabus.courseName + '</td>';
                        content = content + '  <td>' + syllabus.courseCode + '</td>';
                        if (!isEmpty(syllabus.courseProperty)) {
                            var _courseProperty = "";
                            if (syllabus.courseProperty == 0) {
                                _courseProperty = "选修";
                            } else if (syllabus.courseProperty == 1) {
                                _courseProperty = "必修";
                            }
                        }
                        content = content + '  <td>' + _courseProperty + '</td>';
                        content = content + '  <td>' + syllabus.score + '</td>';
    ...
    function deleteSyllabus(id, name) {
        layer.confirm("删除教学大纲: " + name + ", 是否继续？", {icon: 3, title: '提示'}, function (cindex) {
            // 删除数据
            $.ajax({
                url: "${APP_PATH}/syllabusController/deleteSyllabus.do",
                type: "POST",
                data: {
                    id: id
                },
    ...
    function allSel(obj) {
        ...
        var syllabusBox = $("tbody :checkbox");

        $.each(syllabusBox, function (i, n) {
            n.checked = flg;
        });
    }

    function deletesyllabuss() {
        ...
        if (checkBox.length == 0) {
            ...
            layer.confirm("删除选择的角色数据, 是否继续？", {icon: 3, title: '提示'}, function (cindex) {
                ...
                $.ajax({
                    url: "${APP_PATH}/manager/syllabus/deletes.do",
                    type: "POST",
                    data: obj,
                    success: function (result) {
                        if (result.success) {
